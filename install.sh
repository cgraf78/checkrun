  ...
  for ((index = count - 1; index >= 0; index--)); do
    ...
    if [[ "${prior_states[$index]}" == symlink ]]; then
      if ! ln -sfn -- "${prior_targets[$index]}" "$target"; then
        printf 'checkrun: rollback failed to restore symlink: %s\n' "$target" >&2
      fi
    elif ! rm -f "$target"; then
      printf 'checkrun: rollback failed to remove new link: %s\n' "$target" >&2
    fi
  done
...
printf 'installed checkrun commands to %s\n' "$BIN_DIR"
printf 'installed checkrun manpages to %s\n' "$MAN_DIR"
